...
<div class="space-y-8">
  <!-- Header -->
  <div class="border-b border-slate-200 pb-8" in:fade>
    <div class="space-y-2">
      ...
      <h1 class="text-3xl font-bold tracking-tight text-slate-800">Access Patient Records</h1>
      <p class="text-sm text-slate-400">Enter the access key and passcode provided by your patient.</p>
    </div>
  </div>
  ...
  <div class="max-w-lg mx-auto" in:fly={{ y: 20 }}>
    <div class="p-8 rounded-2xl bg-white border border-slate-200 shadow-sm space-y-6">
      <div class="text-center space-y-3">
        ...
        <div class="text-xs font-bold text-slate-700">Verified Patient Access</div>
      ...
      <form on:submit|preventDefault={accessPatient} class="space-y-5">
        <div class="space-y-1.5">
          <label for="shareKey" class="text-[11px] text-slate-500 font-bold px-1">Access Key (UUID)</label>
          <input
            id="shareKey" type="text"
            bind:value={shareKey} required
            class="w-full px-4 py-3 text-sm border border-slate-200 rounded-xl bg-slate-50 focus:bg-white focus:ring-2 focus:ring-emerald-100 focus:border-emerald-300 outline-none transition-all font-mono"
            placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"
          />
        </div>
        <div class="space-y-1.5">
          <label for="passcode" class="text-[11px] text-slate-500 font-bold px-1">Passcode</label>
          <input
            id="passcode" type="password"
            bind:value={passcode} required maxlength="6"
            class="w-full px-4 py-3 text-lg border border-slate-200 rounded-xl bg-slate-50 focus:bg-white focus:ring-2 focus:ring-emerald-100 focus:border-emerald-300 outline-none transition-all font-mono tracking-[0.5em] text-center"
            placeholder="••••"
          />
        </div>
        <button
