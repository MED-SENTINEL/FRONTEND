<script>
  ...
  function renderMarkdown(text) {
    ...
    return text
      ...
      .replace(/^### (.+)$/gm, '<h4 class="text-xs font-black text-sentinel-text dark:text-white mt-3 mb-1 uppercase tracking-wider">$1</h4>')
      .replace(/^## (.+)$/gm, '<h3 class="text-sm font-bold text-sentinel-text dark:text-white mt-3 mb-1">$1</h3>')
      .replace(/^# (.+)$/gm, '<h2 class="text-base font-bold text-sentinel-text dark:text-white mt-3 mb-1">$1</h2>')
      // Bold
      .replace(/\*\*(.+?)\*\*/g, '<strong class="font-bold">$1</strong>')
      // Italic
      .replace(/\*(.+?)\*/g, '<em class="italic opacity-80">$1</em>')
      // Horizontal rules
      .replace(/^---$/gm, '<hr class="my-2 border-slate-200 dark:border-slate-700"/>')
      // Bullet points
      ...
      // Numbered lists
      .replace(/^(\d+)\. (.+)$/gm, '<div class="flex items-start gap-1.5 ml-1"><span class="text-sentinel-dim dark:text-slate-400 font-bold">$1.</span><span>$2</span></div>')
      // Newlines
      .replace(/\n/g, '<br/>');
  }
...
<div class="space-y-8">
  ...
  <div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-slate-200 dark:border-slate-700 pb-8" in:fade>
    <div class="space-y-2">
      ...
      <div class="text-sm text-sentinel-dim dark:text-slate-400 font-medium">
  ...
  {#if errorMsg}
    <div class="hud-panel border-sentinel-critical/30 p-6 flex items-center gap-4 bg-sentinel-critical/5" in:fly={{ y: 10 }}>
      ...
      <div class="text-sm text-sentinel-text dark:text-white">{errorMsg}</div>
    </div>
  {/if}
      ...
      {#each alerts as alert, i}
        ...
        >
          ...
          <div class="flex-1 space-y-1">
            <div class="text-sm font-bold text-sentinel-text dark:text-white">{alert.title}</div>
            <div class="text-xs text-sentinel-dim dark:text-slate-400 leading-relaxed">{alert.body}</div>
            {#if alert.organ_system}
              <div class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 mt-1">
                <span class="text-[9px] font-bold text-sentinel-dim dark:text-slate-400 uppercase">{alert.organ_system}</span>
              </div>
            {/if}
          </div>
        </div>
  ...
  <div class="hud-panel overflow-hidden" in:fade={{ delay: 300 }}>
    <!-- Chat Header -->
    <button 
      class="w-full flex items-center justify-between p-5 bg-gradient-to-r from-sentinel-accent/5 to-purple-500/5 border-b border-slate-200 dark:border-slate-700 hover:from-sentinel-accent/10 hover:to-purple-500/10 transition-colors"
    ...
    >
      <div class="flex items-center gap-3">
        ...
        <div class="text-left">
          <div class="text-sm font-bold text-sentinel-text dark:text-white">LISA — AI Health Assistant</div>
          <div class="text-[10px] text-sentinel-dim dark:text-slate-400">Ask questions about your medical history</div>
        </div>
      </div>
      <svg class="w-4 h-4 text-sentinel-dim dark:text-slate-400 transition-transform {chatOpen ? 'rotate-180' : ''}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    {#if chatOpen}
      <div transition:slide>
        <!-- Chat Messages -->
        <div id="lisa-chat-scroll" class="h-[500px] overflow-y-auto p-4 space-y-3 bg-slate-50/50 dark:bg-sentinel-dark-surface-1/50">
          {#if chatMessages.length === 0}
            <div class="flex flex-col items-center justify-center h-full text-center space-y-4 py-6">
              ...
              <div class="space-y-1">
                <div class="text-sm font-bold text-sentinel-text dark:text-white">Hi! I'm LISA</div>
                <div class="text-xs text-sentinel-dim dark:text-slate-400 max-w-sm">I can answer questions about your medical records. Try one of these:</div>
              </div>
              <div class="flex flex-wrap gap-2 justify-center">
                {#each quickQuestions as q}
                  <button 
                    class="text-[11px] px-3 py-1.5 rounded-full border border-sentinel-accent/20 text-sentinel-accent bg-white dark:bg-sentinel-dark-surface-0 hover:bg-sentinel-accent/5 transition-colors font-medium"
            ...
            {#each chatMessages as msg, i}
              <div class="flex {msg.role === 'user' ? 'justify-end' : 'justify-start'}" in:fly={{ y: 10 }}>
                <div class="max-w-[80%] {msg.role === 'user' 
                  ...
                  : 'bg-white dark:bg-sentinel-dark-surface-0 border border-slate-200 dark:border-slate-700 text-sentinel-text dark:text-white rounded-2xl rounded-bl-md'} px-4 py-3 shadow-sm">
                  ...
                  {/if}
                  {#if msg.context}
                    <div class="text-[9px] mt-2 {msg.role === 'user' ? 'text-white/50' : 'text-sentinel-dim dark:text-slate-400'}">
        ...
        <div class="p-4 border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-sentinel-dark-surface-0">
          <div class="flex gap-2">
            <input
              ...
              class="flex-1 px-4 py-2.5 text-sm border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-sentinel-dark-surface-1 focus:bg-white dark:focus:bg-sentinel-dark-surface-0 focus:ring-2 focus:ring-sentinel-accent/20 focus:border-sentinel-accent/40 outline-none transition-all text-sentinel-text dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500"
            />
            <button
              on:click={sendChat}
  ...
  <!-- ═══ INFO FOOTER ═══ -->
  <div class="hud-panel p-5 bg-slate-50 dark:bg-sentinel-dark-surface-1 border-slate-200 dark:border-slate-700" in:fade={{ delay: 400 }}>
    <div class="flex gap-4 items-start">
      ...
      <div class="space-y-1">
        <div class="text-xs font-bold text-sentinel-text dark:text-white">About SENTINEL AI Agents</div>
