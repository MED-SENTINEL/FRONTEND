<script>
  ...
  import { reports, fetchMyData, loading } from '$lib/stores/data';
  ...
  onMount(() => {
    if ($reports.length === 0) fetchMyData();
  });
</script>
...
<div class="space-y-12">
  <div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-slate-200 dark:border-slate-700 pb-10" in:fade>
    <div class="space-y-2">
      ...
      <div class="text-sm text-sentinel-dim dark:text-slate-400 font-medium">View your complete medical history in chronological order.</div>
    </div>
  </div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-10 lg:gap-16">
    ...
    <div class="lg:col-span-1 space-y-8" in:fade={{ delay: 100 }}>
      ...
      <div class="hud-panel p-6 bg-gradient-to-br from-sentinel-optimal/5 to-transparent border-slate-200 dark:border-slate-700">
        <div class="text-xs font-bold text-sentinel-text dark:text-white mb-4 flex items-center gap-2">
        ...
        <div class="space-y-3">
          <div class="flex justify-between items-center pb-2 border-b border-slate-100 dark:border-slate-700/50">
            <span class="text-xs text-sentinel-dim dark:text-slate-400">Lab Reports</span>
            <span class="text-sm font-bold text-sentinel-text dark:text-white">{$reports.length}</span>
          </div>
    ...
    <div class="lg:col-span-2 relative pl-4 md:pl-8">
      ...
      {#if $loading && timeline.length === 0}
        <div class="py-20 flex flex-col items-center justify-center space-y-4">
          ...
          <div class="text-xs text-sentinel-dim dark:text-slate-400 animate-pulse font-medium">Fetching medical timeline...</div>
        </div>
      {:else}
        <div class="space-y-8 relative z-10">
          {#each timeline as item, i}
            <div class="relative flex items-start gap-6 group" in:fly={{ y: 20, delay: i * 50 }}>
              ...
              <div class="flex flex-col items-center mt-3 shrink-0">
                <div class="w-10 h-10 md:w-12 md:w-12 rounded-full bg-white dark:bg-sentinel-dark-surface-0 border shadow-[0_4px_20px_rgba(15,23,42,0.05)] flex items-center justify-center transition-all duration-300
                  ...
                  : 'border-sentinel-text/20 text-sentinel-text dark:text-white group-hover:border-sentinel-text group-hover:bg-slate-50 dark:bg-sentinel-dark-surface-1'}">
              ...
              <div class="flex-1 min-w-0">
                <div class="text-[11px] font-semibold text-sentinel-dim dark:text-slate-400 mb-2 pl-1">
                ...
                {:else}
                  <div class="hud-panel p-5 bg-white dark:bg-sentinel-dark-surface-0 border border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:border-slate-600 transition-all">
                    <div class="flex justify-between items-start gap-3">
                      <div class="space-y-1">
                        <div class="text-sm font-bold text-sentinel-text dark:text-white uppercase tracking-wider">
                          {item.title || "MEDICAL EVENT"}
                        </div>
                        <div class="flex flex-wrap gap-2 text-[9px] text-sentinel-dim dark:text-slate-400 uppercase tracking-wider">
                          {#if item.trauma_type}
                            <span class="px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700">{item.trauma_type.replace(/_/g, " ")}</span>
                          {/if}
                          {#if item.body_region}
                            <span class="px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700">📍 {item.body_region.replace(/_/g, " ")}</span>
                          {/if}
                          {#if item.severity}
                    ...
                    {#if item.notes}
                      <div class="mt-3 text-xs text-slate-500 leading-relaxed max-w-2xl bg-slate-50 dark:bg-sentinel-dark-surface-1 p-3 rounded-lg border border-slate-100 dark:border-slate-700/50">
                        {item.notes}
                      </div>
                    {/if}
                  </div>
          ...
          {#if timeline.length === 0 && !$loading}
            <div class="hud-panel p-16 text-center opacity-40 border-dashed ml-16 bg-white dark:bg-sentinel-dark-surface-0">
              <div class="text-sm text-sentinel-dim dark:text-slate-400 font-bold">Timeline is empty</div>
